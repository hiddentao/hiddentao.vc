...
const INVESTMENTS = require('./src/data/investments')
...
exports.sourceNodes = ({ actions, createNodeId, createContentDigest, getNodes }) => {
    ...
    if (_isDatoCmsBlogPostNode(node)) {
      ...
      if (defaultLanguage === lang) {
        ...
        versions.push({
          ...
          title,
          date,
          summary: _generateSummary(node.body),
          markdown: node.body,
        })
        ...
        allNodes.forEach(n => {
          if (n.id.startsWith(idPrefix) && n.id !== node.id && n.title) {
            ...
            versions.push({
              ...
              summary: _generateSummary(n.body),
              markdown: n.body,
            })
          }
        })
  ...
  INVESTMENTS.forEach(i => {
    createNode({
      id: createNodeId(i.name),
      ...i,
      date: formatDate(i.date, 'YYYY-MM-DD'),
      internal: {
        mediaType: 'x-investment',
        type: 'Investment',
        contentDigest: createContentDigest(i),
        description: `Investment: ${i.name}`,
      }
    })
  })
}
